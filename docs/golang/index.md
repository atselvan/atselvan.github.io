# Go Programing Language [golang]

> Go is an open source programming language that makes it easy to build simple, reliable, and efficient software.  
Go was created at Google in 2007 by Robert Grisemer, Rob Pike, and Ken Thompson. Go is a compiled, statically typed language.

* [Why Go?](https://blog.smartbear.com/programming/an-introduction-to-the-go-language-boldly-going-where-no-man-has-ever-gone-before/)
* [9 reasons to choose GoLang for your next web application](http://letzgro.net/blog/9-reasons-to-choose-golang-for-your-next-web-application/)
* [Installation](https://golang.org/doc/install)
* [Setting up your workspace](https://golang.org/doc/code.html)

## Important links

* [Getting started with Go](https://github.com/golang/go/wiki)
* [JSON Validator](https://jsonlint.com/)
* [JSON to Go Struct](https://mholt.github.io/json-to-go/)
* [Curl to Go](https://mholt.github.io/curl-to-go/)
* [Pointers Vs Values](https://nathanleclaire.com/blog/2014/08/09/dont-get-bitten-by-pointer-vs-non-pointer-method-receivers-in-golang/)
* [Go for Java programers](https://talks.golang.org/2015/go-for-java-programmers.slide#1)
* [Fetch JSON from an API](http://blog.alexellis.io/golang-json-api-client/)
* [Go Slices (arrays / lists)](https://blog.golang.org/go-slices-usage-and-internals)
* [ForEach loop](https://stackoverflow.com/questions/7782411/is-there-a-foreach-loop-in-go)
* [Go Report card](https://goreportcard.com/)
* [Best Practices](https://talks.golang.org/2013/bestpractices.slide#24)
* [Interfaces in go](https://medium.com/golangspec/interfaces-in-go-part-i-4ae53a97479c)